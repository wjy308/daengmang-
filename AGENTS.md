...
- 로컬 JSON은 임시 파일에 쓴 뒤 rename으로 바꿔치고, 빈 데이터로 시작하는 건 파일이 **없을 때만**
  입니다. 예전엔 저장 중에 폴링이 반쯤 쓰인 파일을 읽으면 파싱 실패 → 빈 데이터로 덮어써
  로컬 데이터가 통째로 날아갔습니다
- 모든 저장은 "전체 읽기 → 수정 → 전체 쓰기"라 **동시에 들어온 저장은 마지막 것만 남습니다**
  (Redis도 동일). 아직 고치지 않았으니 동시 쓰기가 잦은 기능을 넣을 땐 주의하세요
...
바뀌면 모든 캐릭의 `clearedRaids`와 주간 아맞다 체크를 비웁니다. 리셋은 `...data`로 나머지 필드를
전부 보존합니다 — 예전에 필드를 골라 담다가 `customRaids`를 날린 적이 있으니 되돌리지 마세요. 이 키 계산을 복제하는
...
| 종류 | 저장 위치 | 예시 |
|---|---|---|
| 공유 데이터 | Redis / JSON | 유저, 캐릭터, 레이드 배정, 클리어 여부, 골드 기준, 아맞다, 공대장 도구 문구, 직접 클리어 체크 순서 |
| 개인 보기 설정 | localStorage | 공대장 도구 순서, 대시보드 접힘, 가로/카드 배치, **유저 표시 순서**, 제외 캐릭 펼침, 테마, 골드표 보정 |

> **유저 "표시 순서"는 브라우저 설정이고, 캐릭터 순서는 서버 데이터입니다.**
> **직접 클리어 체크의 레이드·유저 순서도 서버 데이터**(`clearPanelOrder`)인데, 이 패널 전용이라
> 대시보드 유저 순서나 레이드 관리 순서와는 따로 놉니다. 공대장 도구 순서는 다시 브라우저 설정입니다.
